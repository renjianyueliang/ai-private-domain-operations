...
$validProfiles = @("read_only", "docs_only", "typecheck")
...
if ($queue.policy.stage -ne 3) { Fail-Validation "policy.stage must be 3." }
...
if ($autoRisks.Count -ne 2 -or $autoRisks -notcontains "read_only" -or $autoRisks -notcontains "low_risk_write") {
  Fail-Validation "Stage 3 allows exactly read_only and low_risk_write for automatic selection."
}
...
[pscustomobject]@{
  selected = $eligible[0]
  reason = "Selected one dependency-satisfied Stage 3 task. The controller did not execute or mutate it."
  max_tasks_per_run = 1
