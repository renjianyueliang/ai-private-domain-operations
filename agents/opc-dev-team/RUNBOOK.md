...
4. 运行 OPC `next`；它只委托原控制器选择最多一个业务任务。
5. 没有选择时停止，不创建任务、不降低风险、不借用其他任务范围。
6. 有选择时，只在任务 allowed_paths 内实现最小改动。
7. 运行变更路径门、固定 verification profile、团队回归和必要的 `git diff --check`。
8. 安全合规复核通过后，只形成候选提交/既有草稿 PR 更新；不合并、不部署。
