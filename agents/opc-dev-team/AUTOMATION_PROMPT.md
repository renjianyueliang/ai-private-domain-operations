...
4. 运行 `-Mode intake`、`-Mode readiness` 和 `-Mode metrics`；重复请求、环境不就绪或边界违规立即停止。
5. 运行 `-Mode next`；没有 selected task 时停止，不创建任务。
6. 只修改 selected task 的 allowed_paths；禁止改控制面、依赖、密钥和部署配置。
7. 运行变更路径门、固定验证、OPC 回归和 `git diff --check`；失败停止。
8. 按 `EVIDENCE_CONTRACT.json` 记录证据；安全合规复核外部动作必须为 0，并记录回滚方法。
9. 只交付候选提交或更新既有草稿 PR；禁止新建重复 PR、force push、合并和部署。
